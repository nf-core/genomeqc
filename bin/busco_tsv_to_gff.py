...
# Converts a BUSCO full_table.tsv into a GFF3 of BUSCO gene locations, plus a
# JSON summary of BUSCO status counts.
...
def main():
    ap = argparse.ArgumentParser(description="Convert a BUSCO full_table.tsv into a GFF3 + JSON stats summary.")
    ap.add_argument("busco_dir", help="Directory containing the BUSCO full_table.tsv (searched recursively)")
    ap.add_argument("sample_id", help="Sample identifier used for output filenames and stats")
    args = ap.parse_args()

    convert_busco_tsv_to_gff(args.busco_dir, args.sample_id)
