...
def validateInputSamplesheet(input) {
    def (meta, refseq, fasta, gff, taxid) = input
    ...
    if ( meta && refseq && !fasta && !gff && taxid ) {
        return [ meta, refseq, taxid ]
    // For the local files option
    } else if ( meta && !refseq && fasta && gff && taxid ) {
        return [ meta, fasta, gff, taxid ]
    } else {
